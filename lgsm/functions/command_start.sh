...
if [ "${output}" == "screen" ]; then
	# Create lockfile
        date '+%s' > "${lockdir}/${selfname}.lock"
        cd "${executabledir}" || exit
	# screen will open the log file when you open the session
	screen -L -Logfile "${consolelog}" -dmS "${sessionname}"
	# dont forget ^M at the end of the command
	screen -S "${sessionname}" -p 0 -X stuff "${executable} ${parms}^M" 
else
...
if [ "${status}" != "0" ]; then
	fn_print_dots "${sessionname}"
	fn_print_info_nl "${sessionname} is already running"
	fn_script_log_error "${sessionname} is already running"
